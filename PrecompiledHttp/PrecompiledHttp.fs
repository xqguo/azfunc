...
module PrecompiledHttp =
  open System.Text
  let cal = System.Globalization.ChineseLunisolarCalendar()
  ...
  let run(req: HttpRequest, log: Microsoft.Extensions.Logging.ILogger) =
      // let d:string = req.QueryString["Date"]
      let res = getFull DateTime.Today
      // ContentResult(Content = res, ContentType = "text/plain", ContentEncoding = Encoding.UTF8)
      res
